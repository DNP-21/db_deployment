...
docker cp tests/check_consistency.js mongo-primary:/tmp/check_consistency.js > /dev/null
...
./simulate_failure.sh stop_primary > /dev/null
...
do
  ...
  docker cp tests/check_consistency.js $node:/tmp/check_consistency.js > /dev/null
...
docker cp tests/test_insert.js $NEW_PRIMARY:/tmp/test_insert.js > /dev/null
docker exec $NEW_PRIMARY mongosh /tmp/test_insert.js

sleep 3

echo "Recovering old primary..."
./simulate_failure.sh recover > /dev/null
